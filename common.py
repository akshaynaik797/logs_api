...
def sms_scheduler():
    ...
    with mysql.connector.connect(**logs_conn_data) as con:
        cur = con.cursor()
        q = "SELECT PatientID_TreatmentID, Type_Ref, Type, status, HospitalID, cdate, person_name,smsTrigger, pushTrigger, `lock`, error, errorDescription FROM hospitalTLog where smsTrigger='0' and `lock`='0' and error='0';"
        cur.execute(q)
        result = cur.fetchall()
    if len(result) > 0:
        for i in result:
